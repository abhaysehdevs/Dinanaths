...
function loadReviews() {
  ...
  reviewsContainer.innerHTML = reviews.length ? '' : '<p>No reviews yet. Be the first to leave one!</p>';
  reviews.forEach(review => {
    reviewsContainer.innerHTML += `
      <div class="review-item">
        <p><strong>${review.name}</strong> - <span class="review-rating">${'⭐'.repeat(review.rating)}</span></p>
        <p>${review.comment}</p>
      </div>
    `;
  });
